...
public class ClinicApplicationService(ClinicDbContext dbContext, ExternalServices.ManagementService managementService, IMemoryCache memoryCache)
{
	public async Task<Consultation> Handle(StartConsultationCommand command)
	{
		...
		PetInfo? petInfo = await memoryCache.GetOrCreateAsync(command.PatientId, async cacheEntry =>
		{
